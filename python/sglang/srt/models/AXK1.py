...
class AttnForwardMethod(IntEnum):
    ...
    # Use sparse multi-latent attention for NPU
    DSA_NPU = auto()
...
def _support_mha_one_shot(attn: AXK1AttentionMLA, forward_batch, backend_name):
...
def _handle_attention_backend(
    attn: AXK1AttentionMLA, forward_batch, backend_name
):
...
class AXK1AttentionMLA(nn.Module):
    ...
    def __init__(
        ...
        if self.nsa_enable_prefill_cp:
            assert self.use_nsa, "CP not supported"
...
class AXK1DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if (
            self.config.n_routed_experts is not None
            and self.layer_id >= self.config.first_k_dense_replace
            and self.layer_id % self.config.moe_layer_freq == 0
        ):
            if hidden_states.shape[0] > 0:
...
class AXK1ForCausalLM(nn.Module):
    ...
    def _maybe_quant_weights_to_fp8_ue8m0(
        ...
        # Quantize some weights to fp8 ue8m0 for A.X K1 nvfp4 checkpoint
        partial_names = []
        nextn_layer_id = (
